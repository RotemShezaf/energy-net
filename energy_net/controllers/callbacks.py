...
class ActionTrackingCallback(BaseCallback):
    ...
    def plot_episode_results(self, episode_num: int, save_path: str):
        ...
        ax1.set_title(f'{self.agent_name} Dispatch & Demand - Episode {episode_num}', fontsize=14)
        ax1.grid(True, alpha=0.3)
        ax1.legend(loc='upper right', fontsize=10)
        # Count timesteps where PCS reduces total demand and the ISO dispatch covers that reduced demand
        good_steps = sum(
        ...
        ax2.set_title('ISO Prices', fontsize=14)
